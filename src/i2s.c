...
const i2s_config i2s_config_default = {48000, 256, 32, 10, 4, 6, 13, false};
...
static bool validate_sck_bck_sync(pio_i2s_clocks* clocks) {
    ...
    printf("Clock speed for SCK: %f (PIO %f Hz with divider %d.%d)\n", (double)actual_sck, (double)clocks->sck_pio_hz, clocks->sck_d, clocks->sck_f);
    printf("Clock speed for BCK: %f (PIO %f Hz with divider %d.%d)\n", (double) actual_bck, (double)clocks->bck_pio_hz, clocks->bck_d, clocks->bck_f);
    printf("Clock Ratio: %f\n", (double)ratio);
    float whole_ratio;
    float fractional_ratio = modff(ratio, &whole_ratio);
    return (fabsf(fractional_ratio) < 1e-6f);
}

static void dma_double_buffer_init(pio_i2s* i2s, void (*dma_handler)(void)) {
    // Set up DMA for PIO I2s - two channels, in and out
    i2s->dma_ch_in_ctrl  = (uint)dma_claim_unused_channel(true);
    i2s->dma_ch_out_ctrl = (uint)dma_claim_unused_channel(true);
    i2s->dma_ch_out_data = (uint)dma_claim_unused_channel(true);
    i2s->dma_ch_in_data  = (uint)dma_claim_unused_channel(true);
...
static void i2s_slave_program_init(PIO pio, const i2s_config* config, pio_i2s* i2s) {
    ...
    if (config->sck_enable) {
        // SCK block
        i2s->sm_sck = (uint8_t)pio_claim_unused_sm(pio, true);
        i2s->sm_mask |= (1u << i2s->sm_sck);
        offset = (uint)pio_add_program(pio, &i2s_sck_program);
        i2s_sck_program_init(pio, i2s->sm_sck, (uint8_t)offset, config->sck_pin);
        pio_sm_set_clkdiv_int_frac(pio, i2s->sm_sck, clocks.sck_d, clocks.sck_f);
    }

    // Bi-Di I2S block, clocked with SCK
    i2s->sm_din  = (uint8_t)pio_claim_unused_sm(pio, true);
    i2s->sm_dout = i2s->sm_din;
    i2s->sm_mask |= (1u << i2s->sm_din);
    offset = (uint)pio_add_program(pio, &i2s_bidi_slave_program);
    i2s_bidi_slave_program_init(pio, i2s->sm_din, (uint8_t)offset, config->dout_pin, config->din_pin);
...
static void i2s_sync_program_init(PIO pio, const i2s_config* config, pio_i2s* i2s) {
    ...
    i2s->sm_din = (uint8_t)pio_claim_unused_sm(pio, true);
    i2s->sm_mask |= (1u << i2s->sm_din);
    offset = (uint)pio_add_program(pio, &i2s_in_slave_program);
    i2s_in_slave_program_init(pio, i2s->sm_din, (uint8_t)offset, config->din_pin);
    pio_sm_set_clkdiv_int_frac(pio, i2s->sm_din, clocks.sck_d, clocks.sck_f);

    // Out block, clocked with BCK
    i2s->sm_dout = (uint8_t)pio_claim_unused_sm(pio, true);
    i2s->sm_mask |= (1u << i2s->sm_dout);
    offset = (uint)pio_add_program(pio, &i2s_out_master_program);
    i2s_out_master_program_init(pio, i2s->sm_dout, (uint8_t)offset, config->bit_depth, config->dout_pin);
